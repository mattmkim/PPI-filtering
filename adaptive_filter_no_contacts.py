...
import numpy as np 
from filter_no_contacts import cluster_pred_interface_atoms, cluster_atom_ind, get_TP_FP, clustering_info_pred_interface_atoms, visualize_post_FPR_TPR
import matplotlib.pyplot as plt
import scipy.spatial as spatial
from sklearn.metrics import auc
from collections import OrderedDict

# modified version of adaptive_filter file to handle proteins with no actual_contact.pdb file
...
max_value = max(roc_area.values())
keys = list({key for key, value in roc_area.items() if value == max_value})
key = max(keys)
...
def visualize_roc_area():
	plt.title('Area Under ROC Curve for Beta Phi Values for MDM2')
	plt.plot(roc_area.keys(), roc_area.values())
	plt.text(150, 3.97, 'Optimal Beta Phi Value: %f' % (float(key) / 100), fontsize=9)
	plt.show()
